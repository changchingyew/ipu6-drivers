...
/* use as such:
echo "0 2 d4xx 1 0x16 0x60 0x48" | sudo tee /sys/kernel/debug/intel-ipu6/isys/delete_device
<csi port> <lanes> <device name> <i2c adapter> <sensor i2c> <ser i2c> <des i2c>
*/
static ssize_t ipu_isys_delete_device_set(struct file *flip,
		const char __user *buffer, size_t len, loff_t *offset)
{
	struct ipu_isys *isys = flip->f_inode->i_private;
	...
	buf[len] = 0;
	dev_info(&isys->adev->dev, "isys_delete_device_set function running val:%s\n", buf);
	res = sscanf(buf, "%d %d %s %d 0x%02x 0x%02x 0x%02x%c", &port, &lanes, name, &adapter, &sens, &ser, &des, &end);
	if (res != 8 && end != '\n') {
		dev_warn(NULL, "%s: Incorrect parameters\n", "delete_device");
		return -EINVAL;
	}
	dev_dbg(&isys->adev->dev, "res:%d, port:%d, lanes:%d, name:%s, adapter:%d, sens:0x%02x, ser:0x%02x, des:0x%02x\n",
		res, port, lanes, name, adapter, sens, ser, des);
	...
	ret = isys_unregister_ext_subdev(isys, sd_info);
	if (!ret)
		ret = len;
...
static ssize_t ipu_isys_delete_device_get(struct file *flip,
		char __user *buffer, size_t len, loff_t *offset)
{
	struct ipu_isys *isys = flip->f_inode->i_private;
	char msg[256] = {0};
	...
	ret = snprintf(msg, sizeof(msg), "IPU CSI2 delete device binding\n"
		"<csi port> <lanes> <device name> <i2c-designware adapter> <sensor i2c> <ser i2c> <des i2c>\n");
	if (copy_to_user(buffer, msg, strlen(msg))) {
		dev_err(&isys->adev->dev, "copy_to_user failed\n");
		ret = -EFAULT;
...
static const struct file_operations isys_delete_device_fops = {
	.read = &ipu_isys_delete_device_get,
	.write = &ipu_isys_delete_device_set,
};
#endif
...
static int ipu_isys_init_debugfs(struct ipu_isys *isys)
{
	...
	file = debugfs_create_file("delete_device", 0600,
		dir, isys, &isys_delete_device_fops);
	if (IS_ERR(file))
		goto err;
#endif
	isys->debugfsdir = dir;
